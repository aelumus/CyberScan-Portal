...
export default function ScanPage() {
    ...
    const handleDrop = useCallback((e: React.DragEvent) => {
        ...
        if (f && (f.name.endsWith(".exe") || f.name.endsWith(".dll"))) { setSelectedFile(f); setErrMsg(""); }
        else setErrMsg("Please upload a .exe or .dll file");
    }, []);

    const submit = async () => {
        if (!selectedFile) { setErrMsg("No file selected"); return; }
        setUploading(true); setErrMsg("");
        const formData = createFormData({ file: selectedFile, use_vt: String(useVt), threshold: String(threshold), mode });
        try {
            const resp = await fetch(buildApiUrl("/api/scan"), { method: "POST", body: formData, headers: authHeaders() });
            if (!resp.ok && resp.status !== 202) throw new Error(await readApiError(resp, `Scan failed (${resp.status})`));
            const scanResp = await resp.json();
            router.push(`/scans/${scanResp.id}`);
        } catch (err: unknown) {
            setErrMsg(err instanceof Error ? err.message : "Backend unreachable");
            setUploading(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-4">
            ...
            <div onClick={() => fileRef.current?.click()}
                ...
                style={{
                    background: dragging ? "rgba(99,102,241,0.1)" : selectedFile ? "rgba(16,185,129,0.06)" : "var(--surface)",
                    border: dragging ? "2px dashed #6366f1" : selectedFile ? "2px dashed #10b981" : "2px dashed var(--border-2)",
                ...
                <input ref={fileRef} type="file" accept=".exe,.dll" className="hidden"
                    onChange={e => { const f = e.target.files?.[0]; if (f) { setSelectedFile(f); setErrMsg(""); } }} />
                {selectedFile ? (
                    <div className="space-y-3">
                        ...
                        <div className="font-bold text-lg" style={{ color: "var(--text)" }}>{selectedFile.name}</div>
                        <div className="text-sm" style={{ color: "var(--text-3)" }}>{(selectedFile.size / 1024).toFixed(1)} KB · Windows PE</div>
            ...
            <button onClick={submit} disabled={uploading || !selectedFile}
