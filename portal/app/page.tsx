...
import { Shield, ArrowRight, Cpu, Lock, Zap, ChevronRight, Activity, AlertTriangle, CheckCircle, BarChart2, Sun, Moon } from "lucide-react";
...
function ParticleCanvas({ light }: { light: boolean }) {
    ...
    useEffect(() => {
        ...
        const dotCol = light ? "rgba(99,102,241,0.3)" : "rgba(99,102,241,0.55)";
        const draw = () => {
...
export default function LandingPage() {
    ...
    useEffect(() => {
        const id = setInterval(() => setTickIdx(t => (t + 1) % TICKER_ITEMS.length), 2800);
        return () => clearInterval(id);
    }, []);

    const cur = TICKER_ITEMS[tickIdx];

    const bg      = light ? "linear-gradient(135deg,#eef2ff 0%,#f5f3ff 50%,#fdf4ff 100%)" : "linear-gradient(135deg,#020617 0%,#0a0f2e 40%,#060d1f 100%)";
    const cardBg  = light ? "rgba(255,255,255,0.85)" : "rgba(15,23,42,0.7)";
    const cardBrd = light ? "rgba(99,102,241,0.15)" : "rgba(255,255,255,0.08)";
    const navBrd  = light ? "rgba(99,102,241,0.1)" : "rgba(255,255,255,0.05)";
    const muted   = light ? "#64748b" : "#94a3b8";
    const dimmed  = light ? "#94a3b8" : "#475569";
    const heading = light ? "#0f172a" : "#ffffff";

    const badgeStyle = (v: string) => {
        if (v === "Malicious") return { background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)", color: "#f87171" };
        if (v === "Suspicious") return { background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", color: "#fbbf24" };
        return { background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.2)", color: "#34d399" };
    };

    const topScore = Math.max(cur.rf, cur.xgb, cur.lgb);
    const scoreColor = topScore >= 60 ? "#f87171" : topScore >= 40 ? "#fbbf24" : "#34d399";
    ...
    return (
        <div className="relative min-h-screen overflow-x-hidden transition-all duration-500" style={{ background: bg }}>
            ...
            <nav className="relative z-10 flex items-center justify-between px-8 py-5 backdrop-blur-sm"
                style={{ borderBottom: `1px solid ${navBrd}` }}>
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-lg tracking-tight" style={{ color: heading }}>
                ...
                <div className="flex items-center gap-2">
                    <button onClick={toggle} className="p-2 rounded-xl transition-all duration-300 hover:scale-110"
                        style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", color: "#818cf8" }}>
                        {light ? <Moon size={15} /> : <Sun size={15} />}
                    </button>
                    <Link href="/scan" className="flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-xl transition-all duration-200"
            ...
            <section className="relative z-10 flex flex-col items-center text-center px-6 pt-28 pb-20">
                ...
                <h1 className="text-5xl md:text-7xl font-black leading-[1.05] tracking-tight mb-6"
                    style={{ color: heading, animation: "fadeUp 0.7s 0.1s ease both" }}>
                    Detect Malware<br />
                ...
                <p className="max-w-xl text-lg leading-relaxed mb-10" style={{ color: muted, animation: "fadeUp 0.7s 0.2s ease both" }}>
                    Static analysis powered by Random Forest, XGBoost and LightGBM.<br />Upload a binary, get a verdict in seconds.
                </p>
                <div className="flex flex-wrap gap-4 justify-center" style={{ animation: "fadeUp 0.7s 0.3s ease both" }}>
                    <Link href="/scan"
                        ...
                        Start Scanning <ArrowRight size={17} className="group-hover:translate-x-1 transition-transform" />
                    </Link>
                    <Link href="/dashboard"
                        className="flex items-center gap-2 px-7 py-3.5 rounded-2xl font-semibold text-sm transition-all duration-200"
                        style={{ border: `1px solid ${cardBrd}`, background: cardBg, color: muted, backdropFilter: "blur(12px)" }}>
                        Dashboard
                    </Link>
                </div>
            </section>
            ...
            <section className="relative z-10 flex justify-center px-6 mb-20">
                <div className="w-full max-w-3xl rounded-2xl overflow-hidden"
                    style={{ background: cardBg, border: `1px solid ${cardBrd}`, backdropFilter: "blur(20px)" }}>
                    <div className="flex items-center gap-2 px-5 py-3 text-xs" style={{ borderBottom: `1px solid ${cardBrd}`, color: dimmed }}>
                        <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /> Live scan feed
                    </div>
                    <div className="px-5 py-4 transition-all duration-700" key={tickIdx}>
                        <div className="flex items-center justify-between flex-wrap gap-3">
                            <div className="flex items-center gap-3">
                                <Activity size={15} style={{ color: dimmed }} />
                                <span className="font-mono text-sm" style={{ color: heading }}>{cur.name}</span>
                                <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={badgeStyle(cur.verdict)}>{cur.verdict}</span>
                            </div>
                        ...
                        <div className="mt-3 grid grid-cols-3 gap-2">
                            {([["RF", cur.rf], ["XGB", cur.xgb], ["LGB", cur.lgb]] as [string, number][]).map(([label, pct]) => (
                                <div key={label}>
                                    <div className="text-xs mb-1" style={{ color: dimmed }}>{label}</div>
                                    <div className="h-1.5 rounded-full overflow-hidden" style={{ background: light ? "rgba(99,102,241,0.1)" : "rgba(255,255,255,0.05)" }}>
                                        <div className="h-full rounded-full transition-all duration-700"
                                            style={{ width: `${pct}%`, background: pct >= 60 ? "linear-gradient(90deg,#ef4444,#dc2626)" : pct >= 40 ? "linear-gradient(90deg,#f59e0b,#d97706)" : "linear-gradient(90deg,#10b981,#059669)" }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={label} className="group relative rounded-2xl p-5 overflow-hidden transition-all duration-300 hover:-translate-y-1"
                            style={{ background: cardBg, border: `1px solid ${cardBrd}`, backdropFilter: "blur(16px)" }}
                            ...
                            <div className="text-3xl font-black mb-1" style={{ color: heading }}><CountUp target={val} suffix={suffix} /></div>
                            <div className="text-sm font-semibold mb-0.5" style={{ color: heading, opacity: 0.8 }}>{label}</div>
                            <div className="text-xs" style={{ color: dimmed }}>{sub}</div>
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="relative z-10 max-w-5xl mx-auto px-6 mb-24">
                <div className="text-center mb-12">
                    <h2 className="text-3xl font-black mb-3" style={{ color: heading }}>
                        Three Models.{" "}
                        <span style={{ background: "linear-gradient(90deg,#818cf8,#c084fc)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>One Verdict.</span>
                    </h2>
                    <p className="text-sm max-w-md mx-auto" style={{ color: muted }}>Ensemble of three algorithms vote on every binary for maximum accuracy.</p>
                </div>
                <div className="grid md:grid-cols-3 gap-5">
                    {[
                        { name: "Random Forest", abbr: "RF",  desc: "Ensemble of 100 decision trees. Robust against overfitting and noisy features.", badge: "scikit-learn", acc: "99.6%", icon: "🌲", glow: "#4f46e5", accent: "#818cf8" },
                        { name: "XGBoost",       abbr: "XGB", desc: "Gradient boosted trees with L1/L2 regularisation. Excels at tabular PE data.", badge: "xgboost.json", acc: "99.6%", icon: "⚡", glow: "#7c3aed", accent: "#a78bfa" },
                        { name: "LightGBM",      abbr: "LGB", desc: "Leaf-wise gradient boost. Fastest inference, high recall on packed samples.", badge: "lgbm.txt",     acc: "99.7%", icon: "🔥", glow: "#ec4899", accent: "#f472b6" },
                    ].map(m => (
                        <div key={m.name} className="group relative rounded-2xl p-6 transition-all duration-300 hover:-translate-y-2 overflow-hidden"
                            ...
                            <div className="flex items-center gap-2 mb-2">
                                <span className="font-bold text-lg" style={{ color: heading }}>{m.name}</span>
                            ...
                            <p className="text-sm leading-relaxed mb-5" style={{ color: muted }}>{m.desc}</p>
                            <div className="flex items-center justify-between text-xs">
                                <span className="font-mono" style={{ color: dimmed }}>{m.badge}</span>
                                <span className="text-emerald-500 font-bold">{m.acc} accuracy</span>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <section className="relative z-10 max-w-4xl mx-auto px-6 mb-24">
                <h2 className="text-2xl font-black text-center mb-10" style={{ color: heading }}>
                ...
                <div className="grid md:grid-cols-4 gap-4">
                    {[
                        ...
                        { step: "02", icon: <Cpu size={20} />, title: "Extract 62 Features", body: "pefile parses PE headers, sections, imports, resources" },
                        ...
                        <div key={step} className="relative rounded-2xl p-5 text-center"
                            style={{ background: cardBg, border: `1px solid ${cardBrd}`, backdropFilter: "blur(12px)" }}>
                            ...
                            <div className="text-xs leading-relaxed" style={{ color: dimmed }}>{body}</div>
                        </div>
                    ))}
                </div>
            </section>
            ...
            <section className="relative z-10 flex flex-col items-center text-center px-6 pb-28">
                <div className="relative rounded-3xl px-12 py-14 overflow-hidden max-w-2xl w-full"
                    style={{ background: cardBg, border: `1px solid ${cardBrd}`, backdropFilter: "blur(24px)" }}>
                    <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at 50% 0%,rgba(99,102,241,0.1),transparent 70%)" }} />
                    <AlertTriangle size={36} className="text-indigo-400 mx-auto mb-5" />
                    <h2 className="text-3xl font-black mb-3" style={{ color: heading }}>Ready to scan?</h2>
                    <p className="mb-8 text-sm max-w-sm mx-auto" style={{ color: muted }}>
                    ...
                    <p className="text-xs mt-6" style={{ color: dimmed }}>Diploma Project · Information Security · 2026</p>
                </div>
            </section>

            <footer className="relative z-10 px-8 py-6 flex items-center justify-between text-xs"
                style={{ borderTop: `1px solid ${navBrd}`, color: dimmed }}>
                <span className="flex items-center gap-2"><Shield size={13} className="text-indigo-400" /> CyberScan Portal</span>
