...
MODEL_REGISTRY = {
    "ds1_rf":   {"name": "Random Forest", "dataset": "DS1 — PE Headers", "algo": "RandomForest", "version": "2.0.0", "status": "active"},
    "ds1_xgb":  {"name": "XGBoost",       "dataset": "DS1 — PE Headers", "algo": "XGBoost",      "version": "2.0.0", "status": "active"},
    "ds1_lgbm": {"name": "LightGBM",      "dataset": "DS1 — PE Headers", "algo": "LightGBM",     "version": "2.0.0", "status": "active"},
}
...
def apply_vt_adjustments(ml_predictions: list[dict], vt_info: dict | None, thr: float) -> tuple[str, float, str]:
                    ...
                    for danger_fn in DANGEROUS_IMPORTS:
                        if func_name.lower().startswith(danger_fn.lower()):
                            dangerous_found.append({
                                "function": func_name,
                                "dll": dll_name,
                                "severity": "high" if danger_fn in HIGH_RISK_IMPORTS else "medium",
                            })
                            break
